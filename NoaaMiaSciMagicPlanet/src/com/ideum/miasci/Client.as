package com.ideum.miasci
{
	...
	public class Client extends EventDispatcher
	{
		...
		{ 			
			...
			{	
				...
				if (message == "READY")
				{
					...
					dispatchEvent(new StringMessageEvent(StringMessageEvent.STRING_RECEIVED, message));							
				}
			...
			{
				trace(e.toString());
				dispatchEvent(new StringMessageEvent(StringMessageEvent.STRING_RECEIVED, e.toString()));												
			}
		}			
		...
		{ 				
			...
			{	
				while(socket.bytesAvailable >= 4)
				{
					...
					//If the socket contains the full message
					if (messageLength <= socket.bytesAvailable) 
					{
						...
						if (typeFlag == TYPE_AMFINT) //AMF object
						{
							...
							if (verbose)
								trace("server sent " + dataMessageInt.toString());
						...
						else if (typeFlag == TYPE_AMFINT2) //AMF object
						{
							...
							if (verbose)
								trace("server sent " + dataMessageInt2.toString());
						...
						else if (typeFlag == TYPE_AMFINT3) //AMF object
						{
							...
							if (verbose)
								trace("server sent " + dataMessageInt3.toString());
						...
						else if (typeFlag == TYPE_AMFBOOL) //AMF object
						{
							...
							if (verbose)
								trace("server sent " + dataMessageBool.toString());
						...
						else if (typeFlag == TYPE_AMFSTR) //AMF object
						{
							...
							if (verbose)
								trace("server sent " + dataMessageStr.toString());
		...
		{
			dispatchEvent(new StringMessageEvent(StringMessageEvent.STRING_RECEIVED, "Connection closed by server."));										
		...
		{
			dispatchEvent(new StringMessageEvent(StringMessageEvent.STRING_RECEIVED, error.text));							
			trace(error.text);
		}
		
		private function securityError(event:SecurityErrorEvent):void
		{
			dispatchEvent(new StringMessageEvent(StringMessageEvent.STRING_RECEIVED, event.text));							
			trace(event.text);
		}
